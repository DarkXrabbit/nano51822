/*
 ...
 2015/6/4	v1.0.2	For nano51822-UDK board.						Jason
 ===============================================================================
 */
...
void onBleEvent(bleDevice * p_ble, BLE_EVENT_T evt) {
	...
	case BLE_ON_DISCONNECTED:	// update adv. data after disconnected.
...
void tskLED(CThread *p_thread, xHandle p_param) {
	...
	while(p_thread->isAlive()) {
		if ( beacon_mode == beacon_mode_normal ) {
			leds = value1;	// output value1 to led bus
			value1 = (value1+1)<0x10 ? (value1+1) : 0;			// count value
		} else {
			leds = value2;	// output value2 to led bus
			value2 = value2 ^ 0x0F;
		}
		sleep(gpUriBeacon->get().beacon_period);
	}
}
